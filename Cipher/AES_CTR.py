...
def aes_ctr_encrypt(flows, key, logfile='./log/AES-CTR.txt', cipherfile=None, keyfile=None, labels=None):
    import csv
    ...
    print(f'AES-128-CTR加密完成，日志已写入{logfile}')
...
def aes_ctr_decrypt(cipher_data, key, logfile='./log/AES-CTR.txt'):
    process = psutil.Process(os.getpid())
    mem_before = process.memory_info().rss / 1024 / 1024  # MB
    max_mem = mem_before
    start = time.perf_counter()
    ...
    print(f'AES-128-CTR解密完成，日志已写入{logfile}')
...
if __name__ == "__main__":
    ...
    logfile = './log/AES-CTR.txt'
    cipherfile = '../Data/AES_CTR/Cipher.csv'
    keyfile = '../Data/AES_CTR/key.txt'
    # 加密并保存
    cipher_data = aes_ctr_encrypt(
        flows, key,
        logfile=logfile,
    ...
    # # 读取密文
    # cipher_data_loaded, loaded_labels = load_cipher_csv(cipherfile)
    # # 解密
    # aes_ctr_decrypt(cipher_data_loaded, key, logfile=logfile)
