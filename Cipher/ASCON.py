...
def ascon_encrypt(flows, key, logfile='./log/ASCON.txt', cipherfile=None, keyfile=None, labels=None):
    import csv
    ...
    print(f'ASCON-128 AEAD加密完成，日志已写入{logfile}')
...
def ascon_decrypt(cipher_data, key, logfile='./log/ASCON.txt'):
    process = psutil.Process(os.getpid())
    mem_before = process.memory_info().rss / 1024 / 1024  # MB
    max_mem = mem_before
    start = time.perf_counter()
    for nonce, ct in cipher_data:
        plain = ascon.decrypt(key, nonce, b'', ct)
        cur_mem = process.memory_info().rss / 1024 / 1024
        if cur_mem > max_mem:
            max_mem = cur_mem
    end = time.perf_counter()
    ...
    print(f'ASCON-128 AEAD解密完成，日志已写入{logfile}')
...
if __name__ == "__main__":
    ...
    logfile = './log/ASCON.txt'
    cipherfile = '../Data/ASCON/Cipher.csv'
    keyfile = '../Data/ASCON/key.txt'
    # 加密并保存
    cipher_data = ascon_encrypt(
        flows, key,
        logfile=logfile,
    ...
    # 解密
    ascon_decrypt(cipher_data_loaded, key_loaded, logfile)
